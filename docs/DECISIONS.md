...
- Fix `Watched first` by updating watched-state selectors, not by touching the perf-era observer filter. Rationale: live console diagnosis showed rows are still `ytd-playlist-video-renderer`, the button is enabled, and the sort handler runs — it found zero watched rows because YouTube renamed the resume-playback overlay (`ytd-…` → `ytw-thumbnail-overlay-resume-playback-renderer`, no `#progress` id) and glues the badge as `WATCHED19:51`, which `\bwatched\b` cannot match; every perf-era content.js change was proven behavior-equivalent to the pre-perf version.
- Add `ytw-thumbnail-overlay-resume-playback-renderer div[style]` as a fourth progress selector, keep the legacy selectors, and relax `hasWatchedText` to `/\bwatched/i`. Rationale: a track div with an inline width is structural and survives YouTube's class-name obfuscation better than the renamed BEM class; `div[style]` still falls through to the rectangle-ratio fallback if the width form changes; the leading `\b` keeps words ending in "watched" from false-positiving while the missing trailing `\b` catches the glued badge.
